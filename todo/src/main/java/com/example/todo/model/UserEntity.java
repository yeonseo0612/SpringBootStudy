...
@Builder //JPA에서 앤티티 클래스로 사용할 것임을 명시
@Entity // lombok의 Builder 패턴을 사용하여 객체를 생성할 수 있도록 해준다
...
public class UserEntity {
	...
	@GeneratedValue(generator="system-uuid")//id 필드의 값을 자동으로 넣는데,
	// 값의 생성은 system-uuid라는 이름을 가진 어노테이션에게 맡긴다.
	@GenericGenerator(name="system-uuid", strategy = "uuid")
	//uuid: 128비트 길이의 고유 식별자
	private String id;	//유저에게 고유하게 부여되는 ID, uuid로 생성되며 완전 고유한 값

	@Column(nullable = false, unique = true) //not null 제약조건
